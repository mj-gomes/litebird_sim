...
def jones_complex_to_polar(j0,j1,j2, integrate_in_band):
    if integrate_in_band:
        return tuple([[[[(np.abs(val), np.angle(val)) for val in row] for row in matrix] for matrix in arr] for arr in [j0,j1,j2]])
    else:
        return tuple([[[(np.abs(val), np.angle(val)) for val in row] for row in matrix] for matrix in [j0,j1,j2]])
    ...
    def fill_tod(
        ...
        for idx_obs, cur_obs in enumerate(obs_list):
            ...
            for idet in range(cur_obs.n_detectors):
                ...
                if self.integrate_in_band:
                    if self.mueller_or_jones == "mueller":
                        assert len(cur_det.mueller_hwp["0f"]) == 1 or len(cur_det.mueller_hwp["2f"]) == 1 or\
                            len(cur_det.mueller_hwp["4f"]) == 1 ,\
                        ...
                        assert len(cur_det.jones_hwp["0f"]) == 1 or len(cur_det.jones_hwp["2f"]) == 1 or\
                            len(cur_det.jones_hwp["4f"]) == 1 ,\
                        "integrate_in_band is set to true but at least one of the harmonics has only one matrix"
                        ...
                        j0f, j2f, j4f = extract_deltas(j0f, j2f, j4f, self.integrate_in_band)
                        j0f, j2f, j4f = jones_complex_to_polar(j0f, j2f, j4f, self.integrate_in_band)
                ...
                if self.build_map_on_the_fly:
                    if self.integrate_in_band_solver:
                        if self.mueller_or_jones == "mueller":
                            assert len(cur_det.mueller_hwp_solver["0f"]) == 1 or len(cur_det.mueller_hwp_solver["2f"]) == 1 or\
                            len(cur_det.mueller_hwp_solver["4f"]) == 1 ,\
                            ...
                            assert len(cur_det.jones_hwp_solver["0f"]) == 1 or len(cur_det.jones_hwp_solver["2f"]) == 1 or\
                            len(cur_det.jones_hwp_solver["4f"]) == 1 ,\
                            "integrate_in_band is set to true but at least one of the harmonics has only one solver matrix"
                            ...
                            j0fs, j2fs, j4fs = extract_deltas(j0fs, j2fs, j4fs, self.integrate_in_band_solver)
                            j0fs, j2fs, j4fs = jones_complex_to_polar(j0fs, j2fs, j4fs, self.integrate_in_band_solver)
